...
list_path <- 'temporario_lista'
### para os decks
deck_path <- 'temporario_deck'
...
decks <- decks %>% 
  mutate(
    categoryName = case_when(is.na(categoryName) & type == 'leader' ~ 'Líder',
                             ...
                             is.na(categoryName) ~ 'Nenhuma',
                             TRUE ~ categoryName)
  )

# pegando apenas as cartas unicas ---------------------------------------------------

## pegando apenas as cartas a partir dos decks
cartas <- decks %>% 
  # removendo as colunas que nao sao relevantes
  select(-deck, -card_in_seq, -repeatCount, -id, -primaryCategoryId, -categoryIds, -secondaryFactions) %>% 
  # adicionando uma contagem de quantas vezes cada carta aparece
  add_count(localizedName, name = 'decks') %>% 
  # dropando as vezes que a carta aparece replicada
  distinct() %>% 
  # recolocando as colunas na ordem que queremos
  select(localizedName, name, short, slug, rarity, cardGroup, type, categoryName, ownable,
         decks, craftingCost, power, provisionsCost, armour, keywords, texto, fluff, small, big) %>% 
  # imputando valores na coluna categoryName para as cartas de lider e estrategia
  mutate(
    categoryName = case_when(is.na(categoryName) & type == 'leader' ~ 'Líder',
                             is.na(categoryName) & type == 'stratagem' ~ 'Estratégia',
                             is.na(categoryName) ~ 'Nenhuma',
    ...
    slug = case_when(slug == 'Northernrealms' ~ 'Northern Realms',
                     ...
                     TRUE ~ slug),
    # ajustando texto da raridade
    rarity = case_when(rarity == 'common' ~ 'Comum',
                       rarity == 'rare' ~ 'Rara',
                       rarity == 'epic' ~ 'Épica',
                       rarity == 'legendary' ~ 'Lendária'),
    # ajustando texto da type
    type = case_when(type == 'artifact' ~ 'Artefato',
                     type == 'leader' ~ 'Líder',
                     type == 'special' ~ 'Especial',
                     type == 'stratagem' ~ 'Estratégia',
                     type == 'unit' ~ 'Unidade')
  )
cartas
...
## salvando as cartas
cartas %>% 
  write_rds(file = str_glue('_posts/2022-01-23-quao-diferentes-sao-as-cartas-de-gwent-de-acordo-com-a-sua-faccao/data/cartas.rds'))
