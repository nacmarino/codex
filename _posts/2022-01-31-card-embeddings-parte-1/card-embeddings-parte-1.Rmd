...
A primeira coisa que vamos olhar são os padrões de ocorrência das palavras entre os tópicos, o que nos permitirá ter uma noção de qual conjunto de palavras melhor caracterizam cada tópico e, consequentemente, do que se trata cada tópico. Esta informação é àquela que está dentro da matriz $\beta$, que descreve a probabilidade de ocorrência de cada palavra em cada tópico e, normalmente, poderia ser extraída usando a função `tidy` especificando o argumento `matrix = 'beta'`. Todavia, esta função não funciona direito para esse propósito quando utilizamos uma covariável para modelar o conteúdo dos tópicos, que foi o que fizemos. Nesse sentido, precisaremos aplicar a função `pluck` ao modelo treinado para extrair as matrizes $\beta$ associadas à cada uma das facções: quando usamos essa covariável, o que fizemos foi 'pedir' para o STM calcular uma matriz $\beta$ para cada uma delas, e utilizar a média dos $\beta$'s entre elas para mapear as palavras associadas à cada tópico. Um ponto importante é que a informação dessas matrizes corresponde ao logarítimo das probabilidades de cada palavra estar associada à cada tópico, além do fato de que os dados chegam em uma estrutura não muito `tidy`.

Depois de tratar àquelas matrizes, podemos observar as 10 palavras com maior probabilidade média de ocorrência em cada tópico na figura abaixo. No geral, podemos observar quatro padrões gerais nos temas que emergem de cada tópico:  
...
+ Existem casos em que os tópicos falam sobre estratégias de jogo, como no caso do Tópico 2 e do Tópico 4: o primeiro fala de uma estratégia de usar a habilidade de escudo para aumentar o poder das cartas e o segundo sobre movimentar cartas para a pilha de descarte e/ou banil-las^[Sei que essa leitura parece um tanto quanto abstrata, mas para quem joga o jogo isso é bastante claro - então peço para que você confie em mim.];  
+ Também é possível observar casos que falam muito sobre um tipo de carta. Isto é especialmente marcante no Tópico 16, onde as palavras-chave remetem diretamente às cartas de cenário: essas são cartas cujas habilidades são disparadas ao longo de três capítulos, cada um dos quais são disparados cada vez que você joga uma carta de um tipo de personagem específico; e,  
+ Existem tópicos que parecem focar exclusivamente nas habilidades e cartas associadas à uma única facção. Isso pode ser observado, por exemplo no Tópico 5, cujas palavras remetem diretamente à alguns padrões sabidos de serem observados entre as cartas da facção `Syndicate`.  
...
Ao que o resultado acima indica, parece não existir uma associação assim tão forte entre as facções e os tópicos: apesar de vermos um ou outro tópico falar muito sobre os temas relacionados à uma facção específica, parece que todo tópico tem alguma coisa de alguma facção. Para validar esse entendimento, podemos utilizar a função `estimateEffect` do pacote `stm`, que ajusta uma regressão para cada tópico através da qual podemos determinar de que forma a proporção dos tópicos varia em função de um conjunto de covariáveis.. 
